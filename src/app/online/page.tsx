...
import { Swords, Plus, LogIn, Users, Cpu } from "lucide-react";
...
export default function OnlineLobbyPage() {
  ...
  const handleJoinRoom = (e: React.FormEvent) => {
    ...
    const room = joinOnlineRoom(joinCode, user.username);
    if (!room) {
  ...
  return (
    <AuthGuard>
      <div className="max-w-4xl mx-auto space-y-8 py-4 select-none">
        {/* Header */}
        <div className="bg-[#090D1F]/90 border border-cyan-500/30 rounded-2xl p-6 shadow-[0_0_30px_rgba(56,189,248,0.1)] flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-12 h-12 rounded-xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center text-cyan-400">
              <Cpu className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-xl font-black text-white">Quantum Room Code Matchmaking</h1>
              <p className="text-xs text-slate-400">Host or join remote 2-player game rooms across devices using 4-digit Room Codes</p>
            </div>
          </div>
        </div>

        {/* Create or Join Room Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#090D1F]/90 border border-cyan-500/30 rounded-2xl p-6 shadow-xl space-y-4 flex flex-col justify-between hover:border-cyan-400 transition">
            <div className="space-y-2">
              ...
              <h2 className="text-lg font-bold text-white">Host New Game Room</h2>
              <p className="text-xs text-slate-400">
                Create a 2-player match room as Player 1 (Cyan). Share your generated 4-digit Room Code with your friend to connect.
              </p>
            </div>
          ...
          <div className="bg-[#090D1F]/90 border border-cyan-500/30 rounded-2xl p-6 shadow-xl space-y-4 flex flex-col justify-between hover:border-fuchsia-400 transition">
            <div className="space-y-2">
              ...
              <h2 className="text-lg font-bold text-white">Join Friend's Room</h2>
              <p className="text-xs text-slate-400">
                Got a Room Code from your friend? Enter it below to join their match as Player 2 (Magenta).
              </p>
            </div>
            ...
            <form onSubmit={handleJoinRoom} className="space-y-3">
              <input
                ...
                className="w-full bg-[#050814] border border-cyan-900/60 rounded-xl px-4 py-3 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-cyan-400 font-mono tracking-wider text-center"
              />
        ...
        {activeRooms.length > 0 && (
          <div className="bg-[#090D1F]/90 border border-cyan-500/30 rounded-2xl p-6 shadow-xl space-y-4">
            <div className="flex items-center space-x-2 border-b border-cyan-900/60 pb-2">
              ...
              <h3 className="text-sm font-bold text-white">Active Open Game Rooms</h3>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {activeRooms.map((room) => (
                ...
                >
                  ...
                  >
                    {room.status === "waiting_for_player2" ? "Waiting P2" : "Playing"}
                  </span>
                </div>
              ))}
            </div>
