...
import { Copy, Check, Users, RefreshCw, ShieldCheck, Swords, LogOut, XCircle, Cpu } from "lucide-react";
...
export default function OnlineRoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleCancelMatch = () => {
    if (!user || !room) return;
    const updated = cancelOnlineRoom(roomId, user.username);
    ...
    return (
      <AuthGuard>
        <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4 text-center">
          ...
          <p className="text-xs text-slate-400">Loading Quantum Room ({roomId})...</p>
        </div>
      </AuthGuard>
    );
  }
      ...
      <div className="space-y-6 py-2 select-none">
        ...
        {room.status === "canceled" && (
          <div className="bg-[#090D1F] border border-red-500/40 rounded-2xl p-10 text-center space-y-4 max-w-md mx-auto shadow-2xl">
            ...
            >
              <span>Return to Cyber Lobby</span>
            </button>
          </div>
        )}
